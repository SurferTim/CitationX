...
var JetEngine = {
    new : func(eng_num){
        ...
        m.running = props.globals.initNode("engines/engine["~eng_num~"]/run",0,"BOOL");
        ...
        m.fuelpump = props.globals.initNode("controls/engines/engine["~eng_num~"]/fuelpump",0,"BOOL");
...
# SurferTim added
setlistener("/controls/engines/engine[0]/fuelpump", func(bp){
    if(!bp.getBoolValue()){
...
# SurferTim added
setlistener("/controls/engines/engine[1]/fuelpump", func(bp){
    if(!bp.getBoolValue()){
...
var Startup = func{
    ...
    setprop("controls/engines/engine[0]/fuelpump",1);
    setprop("controls/engines/engine[1]/fuelpump",1);
...
var Shutdown = func{
    ...
    setprop("controls/engines/engine[0]/cutoff",1);
    setprop("controls/engines/engine[1]/cutoff",1);
    setprop("controls/engines/engine[0]/fuelpump",0);
    setprop("controls/engines/engine[1]/fuelpump",0);
...
var fauxlistener = func{
    ...
    var oat = getprop("environment/temperature-degc")/888;
    if(getprop("engines/engine[0]/fan") > 10.0) {
        var itt = getprop("engines/engine[0]/itt-norm") * 0.695;
        var fan = getprop("engines/engine[0]/fan") * 0.0033;
        setprop("engines/engine[0]/itt-c", itt + fan + oat);
    }  else {
        setprop("engines/engine[0]/itt-c", oat);
    }

    if(getprop("engines/engine[1]/fan") > 10.0) {
        var itt = getprop("engines/engine[1]/itt-norm") * 0.695;
        var fan = getprop("engines/engine[1]/fan") * 0.0033;
        setprop("engines/engine[1]/itt-c", itt + fan + oat);
    }  else {
        setprop("engines/engine[1]/itt-c", oat);
    }

    if((getprop("controls/engines/engine[0]/fuelpump") == 1) and (getprop("engines/engine[0]/fan") > 30)) {
        setprop("engines/engine[0]/run",1);    
   } else {
        setprop("engines/engine[0]/run",0);    
   }

    if((getprop("controls/engines/engine[1]/fuelpump") == 1) and (getprop("engines/engine[1]/fan") > 30)) {
        setprop("engines/engine[1]/run",1);    
        setprop("engines/engine[1]/running",1);    
