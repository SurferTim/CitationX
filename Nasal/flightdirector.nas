...
var FD_set_mode = func(btn){
    ...
    if(btn=="ap"){
        ...
        if(getprop(Vertical_arm) != "GS") setprop(Vertical_arm,"");
...
var nav_src_set=func(src){
    ...
    if(src=="fms"){
        ...
        if(getprop(Lateral) != "HDG") set_roll();
    }
}
...
var course_offset = func(src){
    ...
    crs_offset+=getprop("autopilot/internal/cdi") * 2.0;
    if(crs_offset>180)crs_offset-=360;
    if(crs_offset<-180)crs_offset+=360;
...
var course_offset_true = func(src){
    ...
    if(crs_offset<-180)crs_offset+=360;
    setprop("autopilot/internal/course-offset",crs_offset);
    crs_offset+=getprop("autopilot/internal/cdi") * 1.5;
    if(crs_offset>180)crs_offset-=360;
    if(crs_offset<-180)crs_offset+=360;
    setprop("autopilot/internal/ap_crs",crs_offset);
    setprop("autopilot/internal/selected-crs",crs_set);
...
var monitor_L_armed = func{
    if(getprop(Lateral_arm)!=""){
        if(getprop("autopilot/internal/in-range")){
            ...
            var nav_dist = getprop("autopilot/internal/nav-distance");
            var lat_set = getprop(Lateral);

            if(lat_set == "HDG") {
                if(nav_dist < 20.0) {
                    if(cdi < 20 and cdi > -20){
                        setprop(Lateral,getprop(Lateral_arm));
                        setprop(Lateral_arm,"");
                    }
                } else if(nav_dist < 30.0) {
                    if(cdi < 10 and cdi > -10){
                        setprop(Lateral,getprop(Lateral_arm));
                        setprop(Lateral_arm,"");
                    }
                } else {
...
# SurferTim changed
                if(gs_dst <= 18.0){
                    if(gs_err > -0.10 and gs_err < 0.10){
                       ...
                       interpolate("autopilot/internal/pitch-filter-gain",1.0,5);
                       setprop(Vertical,"GS");
                       setprop(Vertical_arm,"");
                    }  
                }
